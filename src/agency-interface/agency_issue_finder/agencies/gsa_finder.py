...
A specialized issue finder for the General Services Administration (GSA)
that identifies government_services-related issues and prepares research context for Codex CLI.
"""
...
class GSAIssueFinder(AgencyIssueFinder):
    """
    GSA-specific issue finder implementation.
    Identifies government_services-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the GSA issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("GSA", config_dir, data_dir)
        self.government_services_domains = self._load_government_services_domains()
    
    def _load_government_services_domains(self) -> Dict[str, List[str]]:
        """Load government_services domain information."""
        domains_file = os.path.join(self.data_dir, "gsa", "government_services_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any government_services domain
            for domain, subdomains in self.government_services_domains.items():
        ...
        if not self.issues:
            for domain in self.government_services_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "GSA-GEN-001",
                "title": "Government_Services Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between government_services systems and components.",
                "affected_areas": ["All government_services domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "GSA-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-GSA/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific government_services domain.
        
        Args:
            domain: Government_Services domain
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.government_services_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "GSA-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-GSA/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on GSA issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=gsa --issue-id={issue['id']}"
        }
        ...
        for domain in self.government_services_domains:
            ...
            if f"GSA-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the GSA issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="GSA Issue Finder")
    ...
    try:
        # Initialize the GSA issue finder
        finder = GSAIssueFinder(args.config_dir, args.data_dir)
