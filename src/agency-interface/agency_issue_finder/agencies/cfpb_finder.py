...
A specialized issue finder for the Consumer Financial Protection Bureau (CFPB)
that identifies consumer-related issues and prepares research context for Codex CLI.
"""
...
class CFPBIssueFinder(AgencyIssueFinder):
    """
    CFPB-specific issue finder implementation.
    Identifies consumer-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the CFPB issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("CFPB", config_dir, data_dir)
        self.consumer_domains = self._load_consumer_domains()
    
    def _load_consumer_domains(self) -> Dict[str, List[str]]:
        """Load consumer domain information."""
        domains_file = os.path.join(self.data_dir, "cfpb", "consumer_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any consumer domain
            for domain, subdomains in self.consumer_domains.items():
        ...
        if not self.issues:
            for domain in self.consumer_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "CFPB-GEN-001",
                "title": "Consumer Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between consumer systems and components.",
                "affected_areas": ["All consumer domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "CFPB-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-CFPB/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific consumer domain.
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.consumer_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "CFPB-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-CFPB/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on CFPB issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=cfpb --issue-id={issue['id']}"
        }
        ...
        for domain in self.consumer_domains:
            ...
            if f"CFPB-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the CFPB issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="CFPB Issue Finder")
    ...
    try:
        # Initialize the CFPB issue finder
        finder = CFPBIssueFinder(args.config_dir, args.data_dir)
