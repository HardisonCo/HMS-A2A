...
A specialized issue finder for the Nuclear Regulatory Commission (NRC)
that identifies nuclear-related issues and prepares research context for Codex CLI.
"""
...
class NRCIssueFinder(AgencyIssueFinder):
    """
    NRC-specific issue finder implementation.
    Identifies nuclear-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the NRC issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("NRC", config_dir, data_dir)
        self.nuclear_domains = self._load_nuclear_domains()
    
    def _load_nuclear_domains(self) -> Dict[str, List[str]]:
        """Load nuclear domain information."""
        domains_file = os.path.join(self.data_dir, "nrc", "nuclear_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any nuclear domain
            for domain, subdomains in self.nuclear_domains.items():
        ...
        if not self.issues:
            for domain in self.nuclear_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "NRC-GEN-001",
                "title": "Nuclear Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between nuclear systems and components.",
                "affected_areas": ["All nuclear domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "NRC-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-NRC/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific nuclear domain.
        
        Args:
            domain: Nuclear domain
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.nuclear_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "NRC-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-NRC/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on NRC issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=nrc --issue-id={issue['id']}"
        }
        ...
        for domain in self.nuclear_domains:
            ...
            if f"NRC-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the NRC issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="NRC Issue Finder")
    ...
    try:
        # Initialize the NRC issue finder
        finder = NRCIssueFinder(args.config_dir, args.data_dir)
