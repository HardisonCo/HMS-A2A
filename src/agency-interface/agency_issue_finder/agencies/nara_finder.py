...
A specialized issue finder for the National Archives and Records Administration (NARA)
that identifies records-related issues and prepares research context for Codex CLI.
"""
...
class NARAIssueFinder(AgencyIssueFinder):
    """
    NARA-specific issue finder implementation.
    Identifies records-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the NARA issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("NARA", config_dir, data_dir)
        self.records_domains = self._load_records_domains()
    
    def _load_records_domains(self) -> Dict[str, List[str]]:
        """Load records domain information."""
        domains_file = os.path.join(self.data_dir, "nara", "records_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any records domain
            for domain, subdomains in self.records_domains.items():
        ...
        if not self.issues:
            for domain in self.records_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "NARA-GEN-001",
                "title": "Records Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between records systems and components.",
                "affected_areas": ["All records domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "NARA-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-NARA/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific records domain.
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.records_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "NARA-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-NARA/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on NARA issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=nara --issue-id={issue['id']}"
        }
        ...
        for domain in self.records_domains:
            ...
            if f"NARA-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the NARA issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="NARA Issue Finder")
    ...
    try:
        # Initialize the NARA issue finder
        finder = NARAIssueFinder(args.config_dir, args.data_dir)
