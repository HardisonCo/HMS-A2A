...
A specialized issue finder for the Department of Education (ED)
that identifies education-related issues and prepares research context for Codex CLI.
"""
...
class EDIssueFinder(AgencyIssueFinder):
    """
    ED-specific issue finder implementation.
    Identifies education-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the ED issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("ED", config_dir, data_dir)
    ...
    def _load_education_domains(self) -> Dict[str, List[str]]:
        ...
        domains_file = os.path.join(self.data_dir, "ed", "education_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if not self.issues:
            self.issues.append({
                "id": "ED-GEN-001",
                "title": "Education Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between education systems and components.",
                "affected_areas": ["All education domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "ED-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-ED/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        ...
        "civil_rights_education"
            ...
            "resources": [
                {"type": "implementation_plan", "path": "ED-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-ED/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on ED issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=ed --issue-id={issue['id']}"
        }
        ...
        for domain in self.education_domains:
            ...
            if f"ED-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the ED issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="ED Issue Finder")
    ...
    try:
        # Initialize the ED issue finder
        finder = EDIssueFinder(args.config_dir, args.data_dir)
