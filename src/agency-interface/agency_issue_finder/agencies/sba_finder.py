...
A specialized issue finder for the Small Business Administration (SBA)
that identifies business-related issues and prepares research context for Codex CLI.
"""
...
class SBAIssueFinder(AgencyIssueFinder):
    """
    SBA-specific issue finder implementation.
    Identifies business-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the SBA issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("SBA", config_dir, data_dir)
        self.business_domains = self._load_business_domains()
    
    def _load_business_domains(self) -> Dict[str, List[str]]:
        """Load business domain information."""
        domains_file = os.path.join(self.data_dir, "sba", "business_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any business domain
            for domain, subdomains in self.business_domains.items():
        ...
        if not self.issues:
            self.issues.append({
                "id": "SBA-GEN-001",
                "title": "Business Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between business systems and components.",
                "affected_areas": ["All business domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "SBA-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-SBA/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific business domain.
        ...
        "contracting"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.business_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "SBA-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-SBA/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on SBA issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=sba --issue-id={issue['id']}"
        }
        ...
        for domain in self.business_domains:
            ...
            if f"SBA-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the SBA issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="SBA Issue Finder")
    ...
    try:
        # Initialize the SBA issue finder
        finder = SBAIssueFinder(args.config_dir, args.data_dir)
