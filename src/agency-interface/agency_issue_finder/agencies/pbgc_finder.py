...
A specialized issue finder for the Pension Benefit Guaranty Corporation (PBGC)
that identifies pension-related issues and prepares research context for Codex CLI.
"""
...
class PBGCIssueFinder(AgencyIssueFinder):
    """
    PBGC-specific issue finder implementation.
    Identifies pension-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the PBGC issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("PBGC", config_dir, data_dir)
        self.pension_domains = self._load_pension_domains()
    
    def _load_pension_domains(self) -> Dict[str, List[str]]:
        """Load pension domain information."""
        domains_file = os.path.join(self.data_dir, "pbgc", "pension_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any pension domain
            for domain, subdomains in self.pension_domains.items():
        ...
        if not self.issues:
            for domain in self.pension_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "PBGC-GEN-001",
                "title": "Pension Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between pension systems and components.",
                "affected_areas": ["All pension domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "PBGC-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-PBGC/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific pension domain.
        
        Args:
            domain: Pension domain
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.pension_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "PBGC-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-PBGC/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on PBGC issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=pbgc --issue-id={issue['id']}"
        }
        ...
        for domain in self.pension_domains:
            ...
            if f"PBGC-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the PBGC issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="PBGC Issue Finder")
    ...
    try:
        # Initialize the PBGC issue finder
        finder = PBGCIssueFinder(args.config_dir, args.data_dir)
