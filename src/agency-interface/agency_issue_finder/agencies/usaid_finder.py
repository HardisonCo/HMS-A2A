...
A specialized issue finder for the U.S. Agency for International Development (USAID)
that identifies development-related issues and prepares research context for Codex CLI.
"""
...
class USAIDIssueFinder(AgencyIssueFinder):
    """
    USAID-specific issue finder implementation.
    Identifies development-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the USAID issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("USAID", config_dir, data_dir)
        self.development_domains = self._load_development_domains()
    
    def _load_development_domains(self) -> Dict[str, List[str]]:
        """Load development domain information."""
        domains_file = os.path.join(self.data_dir, "usaid", "development_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any development domain
            for domain, subdomains in self.development_domains.items():
        ...
        if not self.issues:
            for domain in self.development_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "USAID-GEN-001",
                "title": "Development Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between development systems and components.",
                "affected_areas": ["All development domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "USAID-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-USAID/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific development domain.
        ...
        "economic_growth"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.development_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "USAID-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-USAID/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on USAID issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=usaid --issue-id={issue['id']}"
        }
        ...
        for domain in self.development_domains:
            ...
            if f"USAID-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the USAID issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="USAID Issue Finder")
    ...
    try:
        # Initialize the USAID issue finder
        finder = USAIDIssueFinder(args.config_dir, args.data_dir)
