...
A specialized issue finder for the Equal Employment Opportunity Commission (EEOC)
that identifies employment_rights-related issues and prepares research context for Codex CLI.
"""
...
class EEOCIssueFinder(AgencyIssueFinder):
    """
    EEOC-specific issue finder implementation.
    Identifies employment_rights-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the EEOC issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("EEOC", config_dir, data_dir)
        self.employment_rights_domains = self._load_employment_rights_domains()
    
    def _load_employment_rights_domains(self) -> Dict[str, List[str]]:
        """Load employment_rights domain information."""
        domains_file = os.path.join(self.data_dir, "eeoc", "employment_rights_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any employment_rights domain
            for domain, subdomains in self.employment_rights_domains.items():
        ...
        if not self.issues:
            for domain in self.employment_rights_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "EEOC-GEN-001",
                "title": "Employment_Rights Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between employment_rights systems and components.",
                "affected_areas": ["All employment_rights domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "EEOC-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-EEOC/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific employment_rights domain.
        
        Args:
            domain: Employment_Rights domain
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.employment_rights_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "EEOC-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-EEOC/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on EEOC issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=eeoc --issue-id={issue['id']}"
        }
        ...
        for domain in self.employment_rights_domains:
            ...
            if f"EEOC-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the EEOC issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="EEOC Issue Finder")
    ...
    try:
        # Initialize the EEOC issue finder
        finder = EEOCIssueFinder(args.config_dir, args.data_dir)
