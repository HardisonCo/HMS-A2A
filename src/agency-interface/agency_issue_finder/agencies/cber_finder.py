...
A specialized issue finder for the Center for Biologics Evaluation and Research (CBER)
...
class CBERIssueFinder(AgencyIssueFinder):
    ...
    def _load_biologics_domains(self) -> Dict[str, List[str]]:
        ...
        domains_file = os.path.join(self.data_dir, "cber", "biologics_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if not self.issues:
            self.issues.append({
                ...
                "resources": [
                    {"type": "implementation_plan", "path": "CBER-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-CBER/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        ...
        "post_market_surveillance"
            ...
            "resources": [
                {"type": "implementation_plan", "path": "CBER-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-CBER/"}
            ]
        })
        ...
        "post_market_surveillance"
            ...
            issues.append({
                ...
                "resources": [
                    {"type": "implementation_plan", "path": "CBER-IMPLEMENTATION-PLAN.md"},
                    {"type": "guidance", "path": "CBER-AI-VALIDATION-GUIDE.md"}
                ]
            })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on CBER issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=cber --issue-id={issue['id']}"
        }
        ...
        for domain in self.biologics_domains:
            ...
            if f"CBER-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the CBER issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="CBER AI Issue Finder")
    ...
    try:
        # Initialize the CBER issue finder
        finder = CBERIssueFinder(args.config_dir, args.data_dir)
