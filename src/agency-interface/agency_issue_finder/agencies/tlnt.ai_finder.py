#!/usr/bin/env python3
"""
tlnt.ai Issue Finder

A specialized issue finder for the TLNT – Talent Management (tlnt.ai)
...
class tlnt.aiIssueFinder(AgencyIssueFinder):
    ...
    def _load_healthcare_domains(self) -> Dict[str, List[str]]:
        ...
        domains_file = os.path.join(self.data_dir, "tlnt.ai", "healthcare_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Find current issues for tlnt.ai.
        ...
        if not self.issues:
            self.issues.append({
                ...
                "resources": [
                    {"type": "implementation_plan", "path": "tlnt.ai-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-tlnt.ai/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        ...
        issues.append({
            "id": f"tlnt.ai-{domain[:2].upper()}-001",
        ...
        "regulatory_compliance"
            ...
            "resources": [
                {"type": "implementation_plan", "path": "tlnt.ai-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-tlnt.ai/"}
            ]
        })
        ...
        "regulatory_compliance"
            ...
            issues.append({
                "id": f"tlnt.ai-{domain[:2].upper()}-002",
                ...
                "resources": [
                    {"type": "implementation_plan", "path": "tlnt.ai-IMPLEMENTATION-PLAN.md"},
                    {"type": "guidance", "path": "tlnt.ai-AI-VALIDATION-GUIDE.md"}
                ]
            })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on tlnt.ai issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=tlnt.ai --issue-id={issue['id']}"
        }
        ...
        for domain in self.healthcare_domains:
            ...
            if f"tlnt.ai-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the tlnt.ai issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="tlnt.ai AI Issue Finder")
    ...
    try:
        # Initialize the tlnt.ai issue finder
        finder = tlnt.aiIssueFinder(args.config_dir, args.data_dir)
