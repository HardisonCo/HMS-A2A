...
A specialized issue finder for the Consumer Product Safety Commission (CPSC)
that identifies consumer_safety-related issues and prepares research context for Codex CLI.
"""
...
class CPSCIssueFinder(AgencyIssueFinder):
    """
    CPSC-specific issue finder implementation.
    Identifies consumer_safety-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the CPSC issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("CPSC", config_dir, data_dir)
        self.consumer_safety_domains = self._load_consumer_safety_domains()
    
    def _load_consumer_safety_domains(self) -> Dict[str, List[str]]:
        """Load consumer_safety domain information."""
        domains_file = os.path.join(self.data_dir, "cpsc", "consumer_safety_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any consumer_safety domain
            for domain, subdomains in self.consumer_safety_domains.items():
        ...
        if not self.issues:
            for domain in self.consumer_safety_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "CPSC-GEN-001",
                "title": "Consumer_Safety Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between consumer_safety systems and components.",
                "affected_areas": ["All consumer_safety domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "CPSC-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-CPSC/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific consumer_safety domain.
        
        Args:
            domain: Consumer_Safety domain
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.consumer_safety_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "CPSC-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-CPSC/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on CPSC issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=cpsc --issue-id={issue['id']}"
        }
        ...
        for domain in self.consumer_safety_domains:
            ...
            if f"CPSC-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the CPSC issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="CPSC Issue Finder")
    ...
    try:
        # Initialize the CPSC issue finder
        finder = CPSCIssueFinder(args.config_dir, args.data_dir)
