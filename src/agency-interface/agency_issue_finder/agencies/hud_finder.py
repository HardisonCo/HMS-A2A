...
A specialized issue finder for the Department of Housing and Urban Development (HUD)
that identifies housing-related issues and prepares research context for Codex CLI.
"""
...
class HUDIssueFinder(AgencyIssueFinder):
    """
    HUD-specific issue finder implementation.
    Identifies housing-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the HUD issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("HUD", config_dir, data_dir)
        self.housing_domains = self._load_housing_domains()
    
    def _load_housing_domains(self) -> Dict[str, List[str]]:
        """Load housing domain information."""
        domains_file = os.path.join(self.data_dir, "hud", "housing_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any housing domain
            for domain, subdomains in self.housing_domains.items():
        ...
        if not self.issues:
            for domain in self.housing_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "HUD-GEN-001",
                "title": "Housing Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between housing systems and components.",
                "affected_areas": ["All housing domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "HUD-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-HUD/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific housing domain.
        ...
        "housing_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.housing_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "HUD-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-HUD/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on HUD issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=hud --issue-id={issue['id']}"
        }
        ...
        for domain in self.housing_domains:
            ...
            if f"HUD-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the HUD issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="HUD Issue Finder")
    ...
    try:
        # Initialize the HUD issue finder
        finder = HUDIssueFinder(args.config_dir, args.data_dir)
