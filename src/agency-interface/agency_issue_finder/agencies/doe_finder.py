...
A specialized issue finder for the Department of Energy (DOE)
that identifies energy-related issues and prepares research context for Codex CLI.
"""
...
class DOEIssueFinder(AgencyIssueFinder):
    """
    DOE-specific issue finder implementation.
    Identifies energy-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the DOE issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("DOE", config_dir, data_dir)
        self.energy_domains = self._load_energy_domains()
    
    def _load_energy_domains(self) -> Dict[str, List[str]]:
        """Load energy domain information."""
        domains_file = os.path.join(self.data_dir, "doe", "energy_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any energy domain
            for domain, subdomains in self.energy_domains.items():
        ...
        if not self.issues:
            for domain in self.energy_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "DOE-GEN-001",
                "title": "Energy Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between energy systems and components.",
                "affected_areas": ["All energy domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "DOE-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-DOE/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific energy domain.
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.energy_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "DOE-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-DOE/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on DOE issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=doe --issue-id={issue['id']}"
        }
        ...
        for domain in self.energy_domains:
            ...
            if f"DOE-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the DOE issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="DOE Issue Finder")
    ...
    try:
        # Initialize the DOE issue finder
        finder = DOEIssueFinder(args.config_dir, args.data_dir)
