...
A specialized issue finder for the Department of State (DOS)
that identifies diplomacy-related issues and prepares research context for Codex CLI.
"""
...
class DOSIssueFinder(AgencyIssueFinder):
    """
    DOS-specific issue finder implementation.
    Identifies diplomacy-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the DOS issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("DOS", config_dir, data_dir)
        self.diplomacy_domains = self._load_diplomacy_domains()
    
    def _load_diplomacy_domains(self) -> Dict[str, List[str]]:
        """Load diplomacy domain information."""
        domains_file = os.path.join(self.data_dir, "dos", "diplomacy_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any diplomacy domain
            for domain, subdomains in self.diplomacy_domains.items():
        ...
        if not self.issues:
            for domain in self.diplomacy_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "DOS-GEN-001",
                "title": "Diplomacy Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between diplomacy systems and components.",
                "affected_areas": ["All diplomacy domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "DOS-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-DOS/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific diplomacy domain.
        
        Args:
            domain: Diplomacy domain
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.diplomacy_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "DOS-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-DOS/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on DOS issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=dos --issue-id={issue['id']}"
        }
        ...
        for domain in self.diplomacy_domains:
            ...
            if f"DOS-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the DOS issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="DOS Issue Finder")
    ...
    try:
        # Initialize the DOS issue finder
        finder = DOSIssueFinder(args.config_dir, args.data_dir)
