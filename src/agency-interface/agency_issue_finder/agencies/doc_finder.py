...
A specialized issue finder for the Department of Commerce (DOC)
that identifies commerce-related issues and prepares research context for Codex CLI.
"""
...
class DOCIssueFinder(AgencyIssueFinder):
    """
    DOC-specific issue finder implementation.
    Identifies commerce-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the DOC issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("DOC", config_dir, data_dir)
        self.commerce_domains = self._load_commerce_domains()
    
    def _load_commerce_domains(self) -> Dict[str, List[str]]:
        """Load commerce domain information."""
        domains_file = os.path.join(self.data_dir, "doc", "commerce_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any commerce domain
            for domain, subdomains in self.commerce_domains.items():
        ...
        if not self.issues:
            for domain in self.commerce_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "DOC-GEN-001",
                "title": "Commerce Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between commerce systems and components.",
                "affected_areas": ["All commerce domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "DOC-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-DOC/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific commerce domain.
        ...
        "trade"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.commerce_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "DOC-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-DOC/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on DOC issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=doc --issue-id={issue['id']}"
        }
        ...
        for domain in self.commerce_domains:
            ...
            if f"DOC-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the DOC issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="DOC Issue Finder")
    ...
    try:
        # Initialize the DOC issue finder
        finder = DOCIssueFinder(args.config_dir, args.data_dir)
