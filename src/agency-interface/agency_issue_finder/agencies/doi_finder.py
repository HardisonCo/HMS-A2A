...
A specialized issue finder for the Department of the Interior (DOI)
that identifies interior-related issues and prepares research context for Codex CLI.
"""
...
class DOIIssueFinder(AgencyIssueFinder):
    """
    DOI-specific issue finder implementation.
    Identifies interior-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the DOI issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("DOI", config_dir, data_dir)
        self.interior_domains = self._load_interior_domains()
    
    def _load_interior_domains(self) -> Dict[str, List[str]]:
        """Load interior domain information."""
        domains_file = os.path.join(self.data_dir, "doi", "interior_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any interior domain
            for domain, subdomains in self.interior_domains.items():
        ...
        if not self.issues:
            for domain in self.interior_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "DOI-GEN-001",
                "title": "Interior Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between interior systems and components.",
                "affected_areas": ["All interior domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "DOI-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-DOI/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific interior domain.
        ...
        "indigenous_affairs"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.interior_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "DOI-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-DOI/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on DOI issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=doi --issue-id={issue['id']}"
        }
        ...
        for domain in self.interior_domains:
            ...
            if f"DOI-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the DOI issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="DOI Issue Finder")
    ...
    try:
        # Initialize the DOI issue finder
        finder = DOIIssueFinder(args.config_dir, args.data_dir)
