...
A specialized issue finder for the Securities and Exchange Commission (SEC)
that identifies securities-related issues and prepares research context for Codex CLI.
"""
...
class SECIssueFinder(AgencyIssueFinder):
    """
    SEC-specific issue finder implementation.
    Identifies securities-related issues and prepares research context.
    """
    
    def __init__(self, config_dir: str, data_dir: str) -> None:
        """
        Initialize the SEC issue finder.
        
        Args:
            config_dir: Directory containing configuration files
            data_dir: Directory containing agency data
        """
        super().__init__("SEC", config_dir, data_dir)
        self.securities_domains = self._load_securities_domains()
    
    def _load_securities_domains(self) -> Dict[str, List[str]]:
        """Load securities domain information."""
        domains_file = os.path.join(self.data_dir, "sec", "securities_domains.json")
    ...
    def find_issues(self, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if topic:
            # Check if topic matches any securities domain
            for domain, subdomains in self.securities_domains.items():
        ...
        if not self.issues:
            for domain in self.securities_domains:
        ...
        if not self.issues:
            self.issues.append({
                "id": "SEC-GEN-001",
                "title": "Securities Systems Integration Analysis",
                "status": "pending",
                "priority": "high",
                "description": "Analysis of integration points between securities systems and components.",
                "affected_areas": ["All securities domains"],
                "detection_date": datetime.now().strftime("%Y-%m-%d"),
                "resources": [
                    {"type": "implementation_plan", "path": "SEC-IMPLEMENTATION-PLAN.md"},
                    {"type": "codebase", "path": "SYSTEM-COMPONENTS/HMS-SEC/"}
                ]
            })
    ...
    def _get_domain_issues(self, domain: str, high_priority_only: bool = False) -> List[Dict[str, Any]]:
        """
        Get issues for a specific securities domain.
        
        Args:
            domain: Securities domain
        ...
        "health_policy"
            ...
            "affected_areas": [subdomain.replace('_', ' ').title() for subdomain in self.securities_domains.get(domain, [])],
            "detection_date": datetime.now().strftime("%Y-%m-%d"),
            "resources": [
                {"type": "implementation_plan", "path": "SEC-IMPLEMENTATION-PLAN.md"},
                {"type": "codebase", "path": f"SYSTEM-COMPONENTS/HMS-SEC/"}
            ]
        })
    ...
    def prepare_context(self, issue_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare context for Codex CLI based on SEC issues.
        ...
        context = {
            ...
            "codex_args": f"--agency=sec --issue-id={issue['id']}"
        }
        ...
        for domain in self.securities_domains:
            ...
            if f"SEC-{domain_code}-" in issue["id"]:
...
def main():
    """Main entry point for the SEC issue finder."""
    import argparse
    
    parser = argparse.ArgumentParser(description="SEC Issue Finder")
    ...
    try:
        # Initialize the SEC issue finder
        finder = SECIssueFinder(args.config_dir, args.data_dir)
